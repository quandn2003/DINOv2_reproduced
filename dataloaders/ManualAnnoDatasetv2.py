...
def get_nii_dataset(config, image_size, **kwargs):
    ...
    norm_func = get_normalize_op(modality='MR', fids=None) # TODO add for ct
    
    test_label = organ_mapping[baseset_name.lower()][config["curr_cls"]]
    base_dir = config['path'][data_name]['data_dir']

    testdataset = ManualAnnoDataset(which_dataset = baseset_name, base_dir=base_dir, idx_split = config['eval_fold'], mode = 'val', scan_per_load = 1, transforms=transforms, min_fg = 1, nsup = config["task"]["n_shots"], fix_length=None, image_size=image_size,extern_normalize_func=norm_func, **kwargs)
